/**
 * Copyright (c) Spectro Cloud
 * SPDX-License-Identifier: Apache-2.0
 */

/**
 * Generated by orval v7.10.0 🍺
 * Do not edit manually.
 * Palette APIs - 4.6
 * OpenAPI spec version: v1
 */
import type { V1LoadBalancerSpecIpAllocationMethod } from "./v1LoadBalancerSpecIpAllocationMethod";
import type { V1LoadBalancerSpecType } from "./v1LoadBalancerSpecType";

/**
 * LoadBalancerSpec defines an Azure load balancer.
 */
export interface V1LoadBalancerSpec {
  apiServerLBStaticIP?: string;
  ipAllocationMethod?: V1LoadBalancerSpecIpAllocationMethod;
  privateDNSName?: string;
  /** PrivateDNSZoneResourceGroup defines the resource group to be used for Azure Private DNS Zone.
If not specified, the resource group of the cluster will be used to create the Azure Private DNS Zone. */
  privateDNSZoneResourceGroup?: string;
  /** Load Balancer type */
  type?: V1LoadBalancerSpecType;
}
